...
(defpackage #:cl-disque-test
  (:use :common-lisp #:rutil #:disque #:should-test)
  (:export #:run-tests))
...
(defun run-tests ()
  (st:test :package '#:cl-disque-test))
              ...
              (let ((client
                     (flex:make-flexi-stream
                      (usocket:socket-stream (usocket:socket-accept
                                              server :element-type
                                              'flex:octet))
                      :external-format cl-disque::+utf8+
...
(deftest connection ()
  (should be null
          (disconnect))
  (should be cl-disque::connection-open-p
          (connect))
  (should signal cl-disque-error
          (connect))
  (should be null
          (disconnect))
  (with-connection ()
    ...
    (should signal cl-disque-error-reply
            (red-auth "pass")))
  ;; QUIT - futile
  )
...
(deftest str-commands ()
  (with-test-db
    ...
    (should be = 16
            (red-setrange "mykey" 10 "Cl-Disque"))
    (should be string= "This is a Cl-Disqueg"
            (red-get "mykey"))
    (should be = 11
            (red-setrange "key2" 6 "Cl-Disque"))
    (should be string= "      Cl-Disque"
            (red-get "key2"))
    (should be string= "Cl-Disqueg"
    ...
    (should be string= "Uhis is a Cl-Disqueg"
            (red-get "mykey"))
    (should be = 16
            (red-strlen "mykey"))
    (should be zerop
...
(deftest server-commands ()
  (with-test-db
    ...
    (should be integerp
            (red:lastsave))
    (should be string= "cl-disque_version"
            (let ((info (red:info)))
              (if (char= #\# (char info 0))
                  (sub info 10 23)
                  (sub info 0 13))))
